...
void FilterPipeline::update_bucket_confidences(const hefkf_common::FrequencyFeatures& freq_features,
    ...
    // Compute quality factor for sharpening
    double quality_factor = compute_quality_factor(freq_features);
    
    // Apply Dirichlet sharpening based on frequency coherence
    sharpen_dirichlet(post_1min, quality_factor);
    sharpen_dirichlet(post_5min, quality_factor);
    
    // Update current bucket confidences for next iteration
    current_bucket_1min_ = post_1min;
    current_bucket_5min_ = post_5min;
}
