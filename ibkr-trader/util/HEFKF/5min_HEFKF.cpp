...
void FiveMinuteHEFKF::setup_5min_noise_matrices() {
    // 5-minute: Larger covariances for smoother behavior
    P_ = Eigen::MatrixXd::Identity(4, 4) * FiveMinHKFConfig::INITIAL_P_SCALE;
    
    // Base measurement noise (optimized for 5min)
    R_base_ << FiveMinHKFConfig::R_PRICE, 0, 0,
               0, FiveMinHKFConfig::R_VOLUME, 0,
               0, 0, FiveMinHKFConfig::R_SPREAD;
    
    // Base process noise (optimized for 5min)
    Q_base_ = Eigen::MatrixXd::Identity(4, 4) * FiveMinHKFConfig::Q_BASE_SCALE;
