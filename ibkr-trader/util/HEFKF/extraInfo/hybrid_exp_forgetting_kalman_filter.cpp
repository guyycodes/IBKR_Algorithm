...
// It is a hybrid of the Exponential Forgetting Kalman Filter and the Kalman Filter
...
// ─────────────────────── Adaptive Lambda Implementation ───────────────────────
double AdaptiveLambda::compute_lambda(double volatility, 
                                     double base_lambda,
                                     double lambda_min,
                                     double lambda_max,
                                     double vol_threshold) {
    // Normalize volatility to [0, 2] range
    double vol_normalized = std::min(volatility / vol_threshold, 2.0);
    
    // Compute adaptive lambda: higher volatility → lower lambda (shorter memory)
    double lambda = lambda_max - (lambda_max - lambda_min) * (vol_normalized / 2.0);
    
    // Clamp result to valid bounds (handles vol spikes that may overshoot)
...
HybridExpForgettingKalmanFilter::HybridExpForgettingKalmanFilter(const HKFConfig& config)
    : config_(config) {
    ...
    Q_ = Eigen::MatrixXd::Identity(4, 4) * 1e-4;
...
void HybridExpForgettingKalmanFilter::setup_matrices(double dt) {
    dt_ = dt;
    ...
    // Base covariances
    P_ = Eigen::MatrixXd::Identity(4, 4) * 1.0;
    ...
    // Base process noise
    Q_base_ = Eigen::MatrixXd::Identity(4, 4) * 1e-4;
    Q_ = Q_base_;
}

void HybridExpForgettingKalmanFilter::initialize(const MarketData& initial_data, double dt) {
...
void HybridExpForgettingKalmanFilter::update_noise(double market_volatility) {
    if (!config_.adaptive_noise) return;
    
    double scale = std::clamp(market_volatility / config_.vol_threshold, 0.5, 5.0);
    ...
    if (std::abs(scale - scale_prev_) > SCALE_UPDATE_THRESHOLD) {
        R_ = R_base_ * scale;
        Q_ = Q_base_ * scale * 0.1;
        scale_prev_ = scale;
    }
}

void HybridExpForgettingKalmanFilter::predict_with_forgetting(double lambda, 
...
void HybridExpForgettingKalmanFilter::update_measurement(const Eigen::VectorXd& measurement) {
    // Innovation: y = z - H * x
...
void HybridExpForgettingKalmanFilter::apply_frequency_nudging(const FrequencyFeatures& freq_features, 
                                                             double price_velocity) {
    if (config_.frequency_domain_weight <= 1e-10) return;
    ...
    if (std::abs(bias) > 1e-10) {
        x_(0) += config_.frequency_domain_weight * bias;
...
double HybridExpForgettingKalmanFilter::compute_control_input(const BucketConfidence& bucket_conf, 
                                                             double price, 
                                                             double& extra_noise) {
    double control_input = 0.0;
    extra_noise = 0.0;
    
    if (config_.bucket_weight <= 1e-10) return control_input;
    ...
    double u_weight = config_.bucket_weight;
...
double HybridExpForgettingKalmanFilter::compute_market_volatility(double price_velocity, double current_price) {
    return current_price > 1e-10 ? std::abs(price_velocity) / current_price : 0.0;
}

FilterOutput HybridExpForgettingKalmanFilter::process(const MarketData& data) {
    if (!initialized_) {
    ...
    // Update noise matrices based on market conditions
    update_noise(market_volatility);
    
    // ---- λ selection ----
    double lambda;
    if (config_.lambda_adaptive) {
        lambda = AdaptiveLambda::compute_lambda(market_volatility,
                                               config_.lambda_fixed,
                                               config_.lambda_min,
                                               config_.lambda_max,
                                               config_.vol_threshold);
    } else {
        lambda = config_.lambda_fixed;
    }
    ...
    if (data.bucket_conf) {
        data.bucket_conf->normalize();  // Normalize first to handle rounding drift
    ...
    // ---- Frequency domain nudging ----
    if (data.freq_features) {
        apply_frequency_nudging(*data.freq_features, price_velocity);
    }
...
void HybridExpForgettingKalmanFilter::set_config(const HKFConfig& config) {
    validate_config(config);
    config_ = config;
}

void HybridExpForgettingKalmanFilter::validate_config(const HKFConfig& config) {
    // Validate lambda_fixed is in valid range (0, 1]
    if (config.lambda_fixed <= 0.0 || config.lambda_fixed > 1.0) {
        throw std::invalid_argument("lambda_fixed must be in range (0, 1], got: " + 
                                  std::to_string(config.lambda_fixed));
    }
    
    // Validate lambda bounds are sensible
    if (config.lambda_min <= 0.0 || config.lambda_min > 1.0) {
        throw std::invalid_argument("lambda_min must be in range (0, 1], got: " + 
                                  std::to_string(config.lambda_min));
    }
    
    if (config.lambda_max <= 0.0 || config.lambda_max > 1.0) {
        throw std::invalid_argument("lambda_max must be in range (0, 1], got: " + 
                                  std::to_string(config.lambda_max));
    }
    
    if (config.lambda_min >= config.lambda_max) {
        throw std::invalid_argument("lambda_min (" + std::to_string(config.lambda_min) + 
                                  ") must be < lambda_max (" + std::to_string(config.lambda_max) + ")");
    }
    
    // Validate other critical parameters
    if (config.vol_threshold <= 0.0) {
        throw std::invalid_argument("vol_threshold must be positive, got: " + 
                                  std::to_string(config.vol_threshold));
    }
    
    if (config.bucket_weight < 0.0 || config.bucket_weight > 1.0) {
        throw std::invalid_argument("bucket_weight must be in range [0, 1], got: " + 
                                  std::to_string(config.bucket_weight));
    }
}

void HybridExpForgettingKalmanFilter::reset() {
    initialized_ = false;
    x_.setZero();
    P_.setIdentity();
    last_price_ = 0.0;
    scale_prev_ = 1.0;  // Reset cached scale factor
    R_ = R_base_;       // Restore base noise matrices
    Q_ = Q_base_;
}
