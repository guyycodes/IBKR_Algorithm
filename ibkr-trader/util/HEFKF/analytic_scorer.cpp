...
const Weights W_BULL{0.40, 0.30, 0.10, 0.05, 0.15};   // Bull: emphasize coherence + trend
const Weights W_BEAR{0.40, 0.30, 0.10, 0.05, 0.15};   // Bear: emphasize coherence + trend  
const Weights W_SIDE{0.60, 0.20, 0.10, 0.05, 0.05};   // Sideways: heavy coherence, light derivatives
...
ScoringResult AnalyticScorer::score_simple(const hefkf_common::FrequencyFeatures& freq_features,
    ...
    ScoringResult result = apply_simple_regime_scoring(freq_features, prior_buckets, simple_regime);
...
double AnalyticScorer::compute_sharpening_factor(const NormalizedFeatures& norm_features, 
    ...
    double base_sharpening = 1.0 + 3.0 * signal_quality * (1.0 - noise_level);
    ...
    switch (regime) {
        ...
        case MarketRegime::BEAR_TREND:
            regime_multiplier = 1.2; // Strong trends increase confidence
            break;
        case MarketRegime::HIGH_VOLATILITY:
            regime_multiplier = 0.8; // High volatility reduces confidence
            break;
        case MarketRegime::LOW_VOLATILITY:
            regime_multiplier = 1.1; // Low volatility slightly increases confidence
            break;
        case MarketRegime::BREAKOUT:
            regime_multiplier = 1.3; // Breakouts have high directional confidence
            break;
        case MarketRegime::REVERSAL:
            regime_multiplier = 0.9; // Reversals are inherently uncertain
            break;
        default:
            regime_multiplier = 1.0;
    }
    
    return std::clamp(base_sharpening * regime_multiplier, 1.0, 5.0);
}
...
void AnalyticScorer::apply_bull_trend_scoring(const NormalizedFeatures& norm, 
    ...
    // Strong upward momentum bias
    bias.up_bias = 1.0 + 0.5 * norm.d_trend;
    bias.down_bias = 1.0 - 0.3 * norm.d_trend;
    
    // Favor larger moves in trending markets
    bias.medium_move_bias = 1.0 + 0.2 * norm.trend;
    bias.large_move_bias = 1.0 + 0.3 * norm.trend;
}

void AnalyticScorer::apply_bear_trend_scoring(const NormalizedFeatures& norm,
                                             double& sharpening, DirectionalBias& bias) const {
    // Strong downward momentum bias
    bias.down_bias = 1.0 + 0.5 * (1.0 - norm.d_trend);
    bias.up_bias = 1.0 - 0.3 * (1.0 - norm.d_trend);
...
hefkf_common::BucketConfidence AnalyticScorer::create_directional_bias(Regime regime, double quality) const {
    // Directional bias vector b - your exact specification
    double bias_mag = 0.25 * quality;   // Max 25% mass re-allocated
...
ScoringResult AnalyticScorer::apply_simple_regime_scoring(const hefkf_common::FrequencyFeatures& freq_features,
    ...
    // Dirichlet sharpening α(q) - your exact specification  
    double alpha = 1.0 + 4.0 * quality;   // Range: [1, 5]
