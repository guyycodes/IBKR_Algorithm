...
// spectral_centroid (volume)   ❌ Computed but unused        ❌ Computed but unused            ❌ No                    ❌ No                     ❌ No
...
// coherence_price_spread_peak  ❌ Computed but unused        ❌ Computed but unused            ❌ No                    ❌ No                     ❌ No

// 📊 Enhanced Quality Signal Breakdown (ALL FEATURES NOW USED):
// Component	        Weight  Features Used	                                                Purpose
// Core Quality	        40%	    coherence_pv × (1-entropy_short)	                            Signal-to-noise ratio
// Trend Quality	    25%	    trend_strength×0.7 + d_trend×0.3	                            Directional stability
// Freq Quality	        20%	    (1-flux)×0.5 + centroid_price×0.3 + (1-centroid_velocity)×0.2	Spectral stability
...
// ✅ FULLY UTILIZED: coherence_pv_peak, spectral_flux, trend_strength_derivative, centroid_velocity, 
//                   spectral_centroid_price, entropy_bands, trend_strength
// ❌ COMPUTED BUT UNUSED: spectral_centroid_volume, coherence_price_spread_peak
// 📈 INTEGRATION STATUS: All major spectral features now actively drive filter parameters and scoring