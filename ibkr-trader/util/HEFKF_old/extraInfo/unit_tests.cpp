...
// Test 5: CSV Data Integration Test
void test_csv_data_integration() {
    TestFramework::print_test_header("CSV Data Integration Test");
    
    // Try to read CSV data from the extraInfo directory
    std::string csv_path = "/workspace/ibkr-trader/util/HEFKF/extraInfo/enhanced_output.csv";
    std::ifstream csv_file(csv_path);
    
    if (!csv_file.is_open()) {
        std::cerr << "ERROR: CSV file not found at " << csv_path << std::endl;
        TestFramework::print_test_result("CSV Data Integration", false);
        throw std::runtime_error("CSV file not found - test failed");
    }
    ...
    while (std::getline(csv_file, line)) {
        ...
        // Expected format: Symbol,Timestamp,Last,Bid,Ask,BidSize,AskSize,Volume,Open,High,Low,Close,Spread,...
        if (fields.size() < 13) continue;
        
        try {
            KalmanTick tick;
            
            // Parse the key fields we need
            long timestamp_ms = std::stol(fields[1]);  // Timestamp
            tick.px = std::stod(fields[2]);            // Last (price)
            tick.bid = std::stod(fields[3]);           // Bid
            tick.ask = std::stod(fields[4]);           // Ask
            tick.volume = std::stod(fields[7]);        // Volume
            tick.spread = std::stod(fields[12]);       // Spread
            tick.trade_count = 1;
            ...
            std::cout << "Warning: Failed to parse CSV line: " << line << std::endl;
            continue;
        }
    }
    ...
    if (ticks.empty()) {
        std::cerr << "ERROR: No valid ticks parsed from CSV" << std::endl;
        TestFramework::print_test_result("CSV Data Integration", false);
        throw std::runtime_error("No valid ticks in CSV - test failed");
    }
    
    std::cout << "Successfully parsed " << ticks.size() << " ticks from CSV" << std::endl;
    
    // Initialize FilterPipeline with 1Hz sampling (adjust as needed)
    FilterPipeline pipeline(1.0);
    
    // Validate all ticks are valid
    TestFramework::assert_true(integration_utils::validate_tick_sequence(ticks), 
                              "CSV tick sequence should be valid");
    ...
    for (size_t i = 1; i < ticks.size(); ++i) {
        try {
            ...
            std::cout << "Warning: Failed to process tick " << i << ": " << e.what() << std::endl;
        }
    }
    
    std::cout << "Successfully processed " << successful_processes << "/" << (ticks.size() - 1) << " ticks" << std::endl;
    std::cout << "Frequency analysis ready for " << freq_ready_count << " ticks" << std::endl;
    
    // Validate results from processed outputs
    if (!outputs.empty()) {
        // Test the last few outputs that should have frequency data
        int valid_outputs = 0;
        double avg_1min_price = 0.0;
        double avg_5min_price = 0.0;
        
        for (size_t i = std::max(0, (int)outputs.size() - 10); i < outputs.size(); ++i) {
            const auto& output = outputs[i];
            
            // Test basic output validity
            TestFramework::assert_true(output.output_1min.price_smoothed > 0, "1min smoothed price should be positive");
            TestFramework::assert_true(output.output_5min.price_smoothed > 0, "5min smoothed price should be positive");
            
            // Test bucket probability validity
            TestFramework::assert_true(output.bucket_conf_1min.is_valid(), "1min bucket confidence should be valid");
            TestFramework::assert_true(output.bucket_conf_5min.is_valid(), "5min bucket confidence should be valid");
            
            // Test lambda values are reasonable
            TestFramework::assert_true(output.output_1min.lambda_used >= 0.5 && output.output_1min.lambda_used <= 1.0,
                                      "1min lambda should be in reasonable range");
            TestFramework::assert_true(output.output_5min.lambda_used >= 0.5 && output.output_5min.lambda_used <= 1.0,
                                      "5min lambda should be in reasonable range");
            
            avg_1min_price += output.output_1min.price_smoothed;
            avg_5min_price += output.output_5min.price_smoothed;
            valid_outputs++;
        }
        
        if (valid_outputs > 0) {
            avg_1min_price /= valid_outputs;
            avg_5min_price /= valid_outputs;
            
            std::cout << "Final validation results:" << std::endl;
            std::cout << "  Average 1min smoothed price: " << std::fixed << std::setprecision(4) << avg_1min_price << std::endl;
            std::cout << "  Average 5min smoothed price: " << std::fixed << std::setprecision(4) << avg_5min_price << std::endl;
            
            // Test that smoothed prices are close to each other (should be similar)
            double price_diff_ratio = std::abs(avg_1min_price - avg_5min_price) / avg_1min_price;
            TestFramework::assert_true(price_diff_ratio < 0.1, "1min and 5min smoothed prices should be similar");
        }
        
        // Test frequency features if available
        if (freq_ready_count > 0) {
            const auto& last_output = outputs.back();
            if (last_output.freq_ready) {
                const auto& freq_features = last_output.freq_features;
                
                // Test that frequency features are in reasonable ranges
                TestFramework::assert_true(freq_features.trend_strength >= 0.0 && freq_features.trend_strength <= 1.0,
                                          "Trend strength should be in [0,1]");
                TestFramework::assert_true(freq_features.coherence_price_volume_peak >= 0.0 && 
                                          freq_features.coherence_price_volume_peak <= 1.0,
                                          "Coherence should be in [0,1]");
                TestFramework::assert_true(freq_features.spectral_flux >= 0.0,
                                          "Spectral flux should be non-negative");
                
                std::cout << "  Final frequency features:" << std::endl;
                std::cout << "    Trend strength: " << freq_features.trend_strength << std::endl;
                std::cout << "    PV coherence peak: " << freq_features.coherence_price_volume_peak << std::endl;
                std::cout << "    Spectral flux: " << freq_features.spectral_flux << std::endl;
            }
        }
    }
    
    TestFramework::print_test_result("CSV Data Integration", true);
}
...
int main() {
    ...
    int tests_passed = 0;
    // int total_tests = 6;
    int total_tests = 1;  // Only running CSV integration test
    ...
    */
    ...
    try {
        test_csv_data_integration();
        tests_passed++;
    } catch (const std::exception& e) {
        std::cerr << "CSV Data Integration test failed: " << e.what() << std::endl;
    }
